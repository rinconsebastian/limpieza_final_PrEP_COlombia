...
```
   ...
   ### 2.1.B early continuation on PrEP   
   
**Numerador:** Número de personas que continuaron PrEP por 3 meses consecutivos después de haber inicidado PrEP en los últimos 12 meses  (80 a 100 dias)
...
numerador <- personas%>%filter(!is.na(`Formulario 3`))%>%
                             mutate(`dias` = as.integer(`Fec. Formulario 3`-`Fec. Formulario 1`))%>%
                             filter(dias >= 80)%>%
                             filter(dias <= 100)%>%
                             group_by(`Población`=Poblacion,Rango=cut(Edad, breaks= c(17,19,24,49,120)) ) %>% 
                             summarise(num= n(), .groups = "keep") %>%
